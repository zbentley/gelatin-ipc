...
use base qw(Class::Accessor);
tutorial::InvalidOperation->mk_accessors( qw( whatOp why ) );
...
sub new {
  ...
  $self->{why} = undef;
  if (UNIVERSAL::isa($vals,'HASH')) {
    if (defined $vals->{whatOp}) {
      $self->{whatOp} = $vals->{whatOp};
    }
    if (defined $vals->{why}) {
      $self->{why} = $vals->{why};
    }
...
sub read {
  ...
  while (1) 
  {
    ...
    {
      /^1$/ && do{      if ($ftype == TType::I32) {
        $xfer += $input->readI32(\$self->{whatOp});
      } else {
        $xfer += $input->skip($ftype);
      }
      last; };
...
sub write {
  ...
  if (defined $self->{whatOp}) {
    $xfer += $output->writeFieldBegin('whatOp', TType::I32, 1);
    $xfer += $output->writeI32($self->{whatOp});
    $xfer += $output->writeFieldEnd();
  }
  if (defined $self->{why}) {
