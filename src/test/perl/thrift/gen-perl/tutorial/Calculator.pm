...
use strict;
use base qw(shared::SharedServiceIf);
...
use strict;
use base qw(shared::SharedServiceRest);
...
use base qw(shared::SharedServiceClient);
...
use strict;
use base qw(shared::SharedServiceProcessor);
...
sub process_calculate {
    ...
    }; if( UNIVERSAL::isa($@,'tutorial::InvalidOperation') ){ 
      $result->{ouch} = $@;
      $@ = undef;
    }
    if ($@) {
      ...
      $output->writeMessageBegin('calculate', TMessageType::EXCEPTION, $seqid);
      $err->write($output);
      $output->writeMessageEnd();
      $output->getTransport()->flush();
      $@ = undef;
